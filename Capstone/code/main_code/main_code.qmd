...
#basemap
basemap <- ggplot() + geom_map(data = map, map = map, aes(long, lat, map_id = region)) + scale_fill_gradient(low = "yellow", high = "purple")
#1900 map
...
quantiles
#emerging <- wpei < 0.238
#developing <- 0.238 < wpei < 0.474
#established <- wpei > 0.474
wemp100yr$category<-as.factor(ifelse(wemp100yr$wpei<0.238, 'emerging',
                     ifelse(wemp100yr$wpei>0.238&wemp100yr$wpei<0.474, 'developing',
                     ifelse(wemp100yr$wpei>0.474, 'established'))))
```
